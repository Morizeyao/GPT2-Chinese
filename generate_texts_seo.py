...
def main():
    ...
    parser.add_argument('--length', default='700 800', type=str, required=False)
    ...
    parser.add_argument('--random_join', default='2 5', type=str, required=False)

    args = parser.parse_args()
    print(args)

    os.environ["CUDA_VISIBLE_DEVICES"] = args.device  # 此处设置程序使用哪些显卡
    lengths = args.length.split()
    ...
    random_join = args.random_join.split()
    ...
    for i, title in enumerate(titles):
        for j in range(articles_per_title):
            with open(save_path + title + '_' + str(j), 'w') as f:
                context_tokens = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(title + '\n'))
                generated = 0
                length = random.randint(int(lengths[0]), int(lengths[1]))
                out = sample_sequence(
                ...
                text = '\n'.join(text.split('\n')[1:-1])
                while text[-1][2] in ['1', '2', '3', '4', '5', '6', '7', '8', '9'] or \
                        text[-1][:2] in ['第一', '第二',
                                         '第三', '第四',
                                         '第五', '第六',
                                         '第七', '第八',
                                         '第九'] or \
                        text[-1][:1] in ['一', '二', '三', '四', '五', '六', '七', '八', '九']:
                    text = text[:-1]

                times = random.randint(int(random_join[0]), int(random_join[1]))
                text = list(text)
